...
export function buildItineraryPlanningPrompt(request: GeminiTripRequest): string {
  ...
  return [
    ...
    '7. MANDATORY TRANSFER BUFFER: You MUST include at least a 20-minute transfer buffer between consecutive stops. The startTime of stop (N) must be at least 20 minutes AFTER stop (N-1) ends (i.e. startTime(N) >= startTime(N-1) + durationMinutes(N-1) + 20 minutes).',
    '8. All stops must start and end strictly within the requested time window (' + request.startTime + ' to ' + request.endTime + ').',
    '9. Give each stop a short reason in Arabic and English explaining why it fits the user preferences.',
    '10. Return only JSON matching the required schema. No markdown or chat text.',
    '',
    'Approved candidates:',
