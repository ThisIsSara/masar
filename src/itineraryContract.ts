...
export function validateGeminiItinerary(
  ...
  for (const [index, stop] of itinerary.stops.entries()) {
    ...
    if (index > 0 && stopStart < previousStopEnd + 20) {
      return `Stop #${index + 1} starts less than 20 minutes after Stop #${index} ends. A minimum 20-minute transfer buffer is required between consecutive stops.`;
    }
